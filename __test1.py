...
def calc(func, tmc, T, chunk=1000, stride=1, limit=1000000, data_folder='', pars=[],
         cnt=0):  # принимает (tmc, T) конкретной траектории, считает для нее func(traj) и сохраняет в data_folder
    run_bool[cnt + 1] = 1
    data = []
    iter = 1
...
def MultiThreading(func, tmcs, Ts=[160, 180], chunk=1000, stride=1, limit=10000000, data_folder='', pars=[], nt=4,
                   recalc=False):  # перебирает значения из tmcs и Ts и запускает для них calc(), nt: number of threads
...
def OneThread(func, tmcs, Ts=[160, 180], chunk=1000, stride=1, limit=10000000, data_folder='', pars=[], nt=1,
              recalc=False):  # перебирает значения из tmcs и Ts и запускает для них calc(), nt: number of threads
    for tmc in tmcs:
        for T in Ts:
            if os.path.exists(nm(tmc, T, 'gro')) and \
